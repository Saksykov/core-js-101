...
function isCreditCardNumber(ccn) {
  let result = ccn.toString();
  let i = 0;
  let item = 0;
  if (ccn.toString().length % 2 !== 0) i = 1;
  for (i; i < ccn.toString().length; i += 2) {
    item = +result[i] * 2;
    if (item > 9) { item -= 9; }
    result = result.slice(0, i).concat(item).concat(result.slice(i + 1));
  }
  return (result.split('').reduce((pv, cv) => +pv + +cv)) % 10 === 0;
}
...
function getDigitalRoot(num) {
  ...
  return getDigitalRoot(num.toString().split('').reduce((a, b) => parseInt(a) + parseInt(b)));
}
